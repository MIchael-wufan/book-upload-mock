...
const cropInset = crop => `inset(${crop.top}% ${crop.right}% ${crop.bottom}% ${crop.left}%)`;
...
function renderRows() {
  $('#bookRows').innerHTML = filteredBooks.map(book => `<tr>
    ...
    <td>${book.newBookId?`<a class="new-book-link" href="https://merc.yuanfudao.com/mark-qs/book/list?source=1&amp;ids=${encodeURIComponent(book.newBookId)}" target="_blank" rel="noopener noreferrer">${book.newBookId}</a>`:'—'}</td></tr>`).join('');
...
function openDetail(id) {
  ...
  $('#editBookInfo').classList.toggle('hidden',!reviewable); $('#bookInfoEditor').classList.add('hidden'); $('#bookInfoDisplay').classList.remove('hidden');
  $('#toggleEdit').classList.toggle('hidden',!reviewable); $('#viewerToolbar').classList.toggle('hidden',!reviewable); $('#decisionActions').classList.toggle('hidden',!reviewable); $('#autosaveText').innerHTML=reviewable?'<span class="dot"></span> 图书信息和图片调整自动保存在本次演示中':'当前任务仅支持查看'; setEditMode(false); renderThumbnails();
}
function renderThumbnails() {
  $('#imageCount').textContent=currentBook.pages.length; const grid=$('#thumbnailGrid'); grid.classList.toggle('editing',editMode);
  grid.innerHTML=currentBook.pages.map((page,i)=>{const sideways=Math.abs(page.rotation)%180===90;return `<div class="thumb ${i===currentImageIndex?'selected':''}" draggable="${editMode}" data-index="${i}"><img class="${sideways?'rotated-sideways':''}" style="transform:rotate(${page.rotation}deg) scale(${sideways?'.72':'1'});clip-path:${cropInset(page.crop)}" src="${page.src}" alt="第 ${i+1} 张答案图片"><span class="thumb-no">${i+1}</span>${isCropped(page.crop)?'<span class="thumb-cropped">已裁切</span>':''}<span class="thumb-drag">⠿</span></div>`;}).join('');
  $$('.thumb').forEach(el=>{
...
function openViewer(){const page=currentBook.pages[currentImageIndex];if(!page)return;const sideways=Math.abs(page.rotation)%180===90;cropDraft=null;$('#cropPanel').classList.add('hidden');$('#cropOverlay').classList.add('hidden');$('#viewerImage').src=page.src;$('#viewerImage').style.transform=`rotate(${page.rotation}deg) scale(${sideways?'.78':'1'})`;$('#viewerImage').style.clipPath=cropInset(page.crop);$('#viewerPosition').textContent=`${currentImageIndex+1} / ${currentBook.pages.length}`;$('#viewerCaption').textContent=`第 ${currentImageIndex+1} 张答案图片 · ${page.rotation===0?'原始方向':`已旋转 ${page.rotation}°`}${isCropped(page.crop)?' · 已裁切':''}`;$('#viewerModal').classList.remove('hidden');updateViewerButtons();}
...
function updateCropOverlay(){if(!cropDraft)return;const imageRect=$('#viewerImage').getBoundingClientRect(),canvasRect=$('#viewerCanvas').getBoundingClientRect(),overlay=$('#cropOverlay');const left=imageRect.left-canvasRect.left+imageRect.width*cropDraft.left/100,top=imageRect.top-canvasRect.top+imageRect.height*cropDraft.top/100,width=imageRect.width*(100-cropDraft.left-cropDraft.right)/100,height=imageRect.height*(100-cropDraft.top-cropDraft.bottom)/100;Object.assign(overlay.style,{left:`${left}px`,top:`${top}px`,width:`${width}px`,height:`${height}px`});$('#cropDimensions').textContent=`${Math.round(100-cropDraft.left-cropDraft.right)}% × ${Math.round(100-cropDraft.top-cropDraft.bottom)}%`;}
function openCropPanel(){const page=currentBook.pages[currentImageIndex];if(!page)return;cropDraft={...page.crop};$('#viewerImage').style.clipPath='none';$('#cropPanel').classList.remove('hidden');$('#cropOverlay').classList.remove('hidden');requestAnimationFrame(updateCropOverlay);}
function closeCropPanel(){cropDraft=null;$('#cropPanel').classList.add('hidden');$('#cropOverlay').classList.add('hidden');}
function startCropDrag(event){if(!cropDraft)return;event.preventDefault();const handle=event.target.dataset.handle||'move',imageRect=$('#viewerImage').getBoundingClientRect(),startX=event.clientX,startY=event.clientY,start={...cropDraft},minSize=12;const onMove=moveEvent=>{const dx=(moveEvent.clientX-startX)/imageRect.width*100,dy=(moveEvent.clientY-startY)/imageRect.height*100;let next={...start};if(handle==='move'){const moveX=clamp(dx,-start.left,start.right),moveY=clamp(dy,-start.top,start.bottom);next.left=start.left+moveX;next.right=start.right-moveX;next.top=start.top+moveY;next.bottom=start.bottom-moveY;}else{if(handle.includes('w'))next.left=clamp(start.left+dx,0,100-start.right-minSize);if(handle.includes('e'))next.right=clamp(start.right-dx,0,100-start.left-minSize);if(handle.includes('n'))next.top=clamp(start.top+dy,0,100-start.bottom-minSize);if(handle.includes('s'))next.bottom=clamp(start.bottom-dy,0,100-start.top-minSize);}cropDraft=next;updateCropOverlay();};const onUp=()=>{document.removeEventListener('mousemove',onMove);document.removeEventListener('mouseup',onUp);};document.addEventListener('mousemove',onMove);document.addEventListener('mouseup',onUp);}
...
function showConfirm(type){pendingDecision=type;const pass=type==='pass';$('#confirmIcon').textContent=pass?'✓':'×';$('#confirmIcon').className=`confirm-icon ${pass?'':'reject'}`;$('#confirmTitle').textContent=pass?'确认通过审核？':'确认拒绝该图书？';$('#confirmText').textContent=pass?`通过后将把“${currentBook.uploader}”记录为图书上传者。`:'拒绝后将同步状态给 C 端，请填写审核备注。';$('#rejectRemarkField').classList.toggle('hidden',pass);$('#rejectRemark').value='';$('#submitConfirm').textContent=pass?'确认通过':'确认拒绝';$('#submitConfirm').className=`btn ${pass?'success':'danger'}`;$('#confirmModal').classList.remove('hidden');if(!pass)$('#rejectRemark').focus();}
...
$('#editBookInfo').addEventListener('click',()=>{fillBookEditor();$('#bookInfoDisplay').classList.add('hidden');$('#bookInfoEditor').classList.remove('hidden');});
$('#cancelBookInfo').addEventListener('click',()=>{$('#bookInfoEditor').classList.add('hidden');$('#bookInfoDisplay').classList.remove('hidden');});
$('#saveBookInfo').addEventListener('click',()=>{const name=$('#editBookName').value.trim(),subtitle=$('#editBookSubtitle').value.trim();if(!name||!subtitle){showToast('图书名称和册次不能为空');return;}Object.assign(currentBook,{name,subtitle,grade:$('#editBookGrade').value,course:$('#editBookCourse').value});refreshBookDetails();renderRows();$('#bookInfoEditor').classList.add('hidden');$('#bookInfoDisplay').classList.remove('hidden');showToast('图书基本信息已保存');});
...
$('#resetCrop').addEventListener('click',()=>{cropDraft={top:0,right:0,bottom:0,left:0};updateCropOverlay();});
$('#cancelCrop').addEventListener('click',()=>{closeCropPanel();openViewer();});
$('#applyCrop').addEventListener('click',()=>{const appliedCrop={...cropDraft};currentBook.pages[currentImageIndex].crop=appliedCrop;closeCropPanel();renderThumbnails();openViewer();showToast(isCropped(appliedCrop)?'图片裁切已应用':'图片裁切已重置');});
window.addEventListener('resize',()=>{if(!$('#cropOverlay').classList.contains('hidden'))updateCropOverlay();});
...
$('#passBook').addEventListener('click',()=>{if(!$('#bookInfoEditor').classList.contains('hidden')){showToast('请先保存或取消图书信息编辑');return;}showConfirm('pass');}); $('#rejectBook').addEventListener('click',()=>{if(!$('#bookInfoEditor').classList.contains('hidden')){showToast('请先保存或取消图书信息编辑');return;}showConfirm('reject');});
$('#cancelConfirm').addEventListener('click',()=>$('#confirmModal').classList.add('hidden')); $('#submitConfirm').addEventListener('click',()=>{const rejectRemark=$('#rejectRemark').value.trim();if(pendingDecision==='reject'&&!rejectRemark){showToast('请填写审核备注');$('#rejectRemark').focus();return;}currentBook.status=pendingDecision==='pass'?'审核通过':'人工审核未通过';currentBook.remark=pendingDecision==='pass'?'':rejectRemark;if(pendingDecision==='pass'&&!currentBook.newBookId)currentBook.newBookId=String(5048700+books.indexOf(currentBook));$('#confirmModal').classList.add('hidden');const st=$('#detailStatus');st.textContent=currentBook.status;st.className=`status ${statusClass[currentBook.status]}`;$('#toggleEdit').classList.add('hidden');$('#viewerToolbar').classList.add('hidden');$('#decisionActions').classList.add('hidden');$('#autosaveText').textContent='当前任务仅支持查看';setEditMode(false);updateSummaries();showToast(pendingDecision==='pass'?`审核通过并完成入库，新图书 ID：${currentBook.newBookId}`:'人工审核未通过，已记录审核备注');});
