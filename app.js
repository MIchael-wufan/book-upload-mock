...
function fillBookEditor(){$('#editBookName').value=currentBook.name;$('#editBookSubtitle').value=currentBook.subtitle;$('#editBookIsbn').value=currentBook.isbn;$('#editBookGrade').value=currentBook.grade;$('#editBookCourse').value=currentBook.course;$('#editBookProvince').value=currentBook.province;}
...
$('#saveBookInfo').addEventListener('click',()=>{const name=$('#editBookName').value.trim(),subtitle=$('#editBookSubtitle').value.trim(),isbn=$('#editBookIsbn').value.trim(),normalizedIsbn=isbn.replace(/-/g,'');if(!name||!subtitle||!isbn){showToast('图书名称、册次和 ISBN 不能为空');return;}if(!/^(?:\d{10}|\d{13})$/.test(normalizedIsbn)){showToast('ISBN 需为 10 位或 13 位数字');$('#editBookIsbn').focus();return;}Object.assign(currentBook,{name,subtitle,isbn:normalizedIsbn,grade:$('#editBookGrade').value,course:$('#editBookCourse').value,province:$('#editBookProvince').value});refreshBookDetails();renderRows();$('#bookInfoEditor').classList.add('hidden');$('#bookInfoDisplay').classList.remove('hidden');showToast('图书基本信息已保存');});
...
$('#resetCrop').addEventListener('click',()=>{cropDraft={top:0,right:0,bottom:0,left:0};syncCropControls();});
$('#cancelCrop').addEventListener('click',()=>{openViewer();});
$('#applyCrop').addEventListener('click',()=>{currentBook.pages[currentImageIndex].crop={...cropDraft};renderThumbnails();openViewer();showToast(isCropped(cropDraft)?'图片裁切已应用':'图片裁切已重置');});
...
$('#passBook').addEventListener('click',()=>{if(!$('#bookInfoEditor').classList.contains('hidden')){showToast('请先保存或取消图书信息编辑');return;}showConfirm('pass');}); $('#rejectBook').addEventListener('click',()=>{if(!$('#bookInfoEditor').classList.contains('hidden')){showToast('请先保存或取消图书信息编辑');return;}if(!$('#reviewRemark').value.trim()){showToast('请先填写拒绝原因');$('#reviewRemark').focus();return;}showConfirm('reject');});
$('#cancelConfirm').addEventListener('click',()=>$('#confirmModal').classList.add('hidden')); $('#submitConfirm').addEventListener('click',()=>{currentBook.status=pendingDecision==='pass'?'审核通过':'人工审核未通过';currentBook.remark=pendingDecision==='pass'?'':$('#reviewRemark').value.trim();if(pendingDecision==='pass'&&!currentBook.newBookId)currentBook.newBookId=String(5048700+books.indexOf(currentBook));$('#confirmModal').classList.add('hidden');const st=$('#detailStatus');st.textContent=currentBook.status;st.className=`status ${statusClass[currentBook.status]}`;$('#reviewRemark').disabled=true;$('#toggleEdit').classList.add('hidden');$('#viewerToolbar').classList.add('hidden');$('#decisionActions').classList.add('hidden');$('#autosaveText').textContent='当前任务仅支持查看';setEditMode(false);updateSummaries();showToast(pendingDecision==='pass'?`审核通过并完成入库，新图书 ID：${currentBook.newBookId}`:'人工审核未通过，已记录审核备注');});
